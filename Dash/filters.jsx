// Sistema global de filtros — compartilhado entre Visão geral, Pré-vendas e Vendas
// Persiste no localStorage e usa custom event para broadcast entre componentes.
...
// ── Hook: observa o estado global ────────────────
function useFilters() {
  const [state, setState] = React.useState(() => loadFilters());
  React.useEffect(() => {
...
// ── Select customizado (pill discreto) ───────────
function PillSelect({ icon, label, value, opts, onChange, width }) {
  ...
  return (
    <div className={`fbar-pill ${active ? 'is-active' : ''} ${open ? 'is-open' : ''}`} ref={rootRef} style={{ minWidth: width }}>
...
// ── Segmento de período (Dia, Semana, Mês...) ────
        ...
        <footer className="fbar-drawer-foot">
          <button className="fbar-btn-ghost" onClick={() => {
            ['produto','etapa','status','tipoReceita','ticketFaixa','atividade'].forEach(k => {
              const defaultVal = k === 'etapa' ? 'todas' : 'todos';
              onUpdate({ [k]: defaultVal });
            });
  ...
  return (
    <div className="fbar">
      <div className="fbar-main">
        ...
        {children /* extras per page (ex: botão exportar) */}
      </div>
...
// expor globalmente para que as três páginas usem
Object.assign(window, {
  useFilters, FilterBar, loadFilters, FILTER_DEFAULTS,
  OPT_PERIOD, OPT_CANAL, OPT_RESP, OPT_PRODUTO, OPT_ETAPA,
  OPT_STATUS, OPT_TIPO_RECEITA, OPT_TICKET, OPT_ATIVIDADE,
});
