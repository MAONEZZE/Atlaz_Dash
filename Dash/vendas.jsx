// Vendas — Central de controle financeiro e comercial
const { useState: useV, useMemo: useVM } = React;
...
// ── Tabela financeira mensal (planilha-like) ─────
      ...
      <table className="fin-tbl">
        <thead>
          <tr>
            ...
            {meses.map((m, i) => (
              <th key={m} className={`fin-tbl-mo ${i === mesAtualIdx ? 'is-atual' : ''}`}>
                {m}
              </th>
            ))}
          ...
          {linhas.map((ln) => (
            <tr key={ln.id} className={`fin-tbl-row tipo-${ln.tipo}`}>
              ...
              {ln.valores.map((v, i) => (
                <td
                  key={i}
                  className={`fin-tbl-val num ${i === mesAtualIdx ? 'is-atual' : ''} ${v === 0 ? 'is-zero' : ''}`}
                >
                  {fmt(v)}
                </td>
              ))}
...
// ── Stat chip (secundário, denso) ────────────────
function StatChip({ label, value, tone, icon, fmt }) {
  return (
    <div className={`stat-chip ${tone ? 'chip-' + tone : ''}`}>
      ...
      <div className={`stat-chip-value num ${fmt || ''}`}>{value}</div>
    </div>
  );
}

// ── KPI card (financeiro) ────────────────────────
function FinKpi({ label, value, delta, sub, tone, big, suffix, delay = 0 }) {
  const up = delta > 0;
...
// ── Gráfico mensal (barras bruto vs liquido + linha previsto) ──
    ...
    <svg className="mensal-chart" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet">
      <defs>
        <linearGradient id="brutoGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          <stop offset="100%" stopColor="#2551D8" />
        </linearGradient>
      ...
      {tickVals.map((v, i) => (
        <g key={i}>
          ...
          <text x={padL - 10} y={y(v) + 3} textAnchor="end"
            className="axis-txt">{fmtVBR(v, true)}</text>
        </g>
      ))}
        ...
        return (
          <g key={i}>
            ...
            <rect
              ...
              y={y(d.liquido)}
              width={barW}
              height={chartH - (y(d.liquido) - padT)}
              rx="3"
              fill="url(#liqGrad)"
              opacity={isAtual ? 1 : 0.85}
            />
            {isAtual && (
              <g>
                <rect x={cx - colW/2 + 4} y={padT} width={colW - 8} height={chartH}
                  fill="#2551D8" opacity="0.04" rx="4" />
                <text x={cx} y={y(d.bruto) - 10} textAnchor="middle"
                  className="axis-txt" fill="#2551D8" style={{ fontWeight: 600 }}>
                  {fmtVBR(d.bruto, true)}
                </text>
              </g>
            )}
            <text x={cx} y={H - padB + 20} textAnchor="middle"
              className="axis-txt" fill={isAtual ? 'var(--n-900)' : 'var(--n-500)'}
              style={{ fontWeight: isAtual ? 600 : 400 }}>
              {d.m}
            </text>
          </g>
        );
      ...
      {data.map((d, i) => (
        <circle key={i} cx={x(i)} cy={y(d.previsto)} r="2.2" fill="#10B981" />
      ))}
    </svg>
  );
}
        ...
        return (
          <div key={c.id} className="canal-row">
            <div className="canal-row-head">
              <div className="canal-row-name">
                <span className="leg-dot" style={{ background: c.cor }} />
                {c.nome}
              </div>
              <div className="canal-row-meta">
                <span className="num">{fmtVBR(c.bruto, true)}</span>
                <span className="canal-pct num">{pct.toFixed(1)}%</span>
              </div>
            </div>
            <div className="canal-row-bar">
              <div className="canal-row-fill" style={{ width: `${w}%`, background: c.cor }} />
            </div>
...
// ── Breakdown financeiro (mini waterfall) ────────
...
function AppVendas() {
  const [period, setPeriod] = useV('mes');

  const liquidoDelta = FIN_RESUMO.deltaLiquido;
  const brutoDelta   = FIN_RESUMO.deltaBruto;
  const vendasDelta  = FIN_RESUMO.deltaVendas;
  const margemDelta  = FIN_RESUMO.deltaMargem;
    ...
    <div className="dash">
      ...
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <nav className="topnav">
            ...
            <a className="item" href="Pré-vendas.html">Pré-vendas</a>
      ...
      <main className="main">
        ...
        <div className="fin-kpi-grid" style={{ marginBottom: 32 }}>
          <FinKpi
            label="Receita bruta"
            value={fmtVBR(FIN_RESUMO.bruto)}
            delta={brutoDelta}
            sub="antes de taxas & impostos"
            tone="hero"
            delay={0}
          />
          <FinKpi
            label="Faturamento Bruto Vendido"
            value={fmtVBR(FIN_RESUMO.liquido)}
            delta={liquidoDelta}
            sub="no mês"
            delay={40}
          />
          <FinKpi
            label="Líquido Vendido no Mês"
            value={FIN_RESUMO.margem.toFixed(1).replace('.', ',')}
            suffix="%"
            delta={margemDelta}
            sub="líquido / bruto"
            delay={80}
          ...
          <FinKpi
            label="Vendas"
            value={fmtNum(FIN_RESUMO.vendas)}
            delta={vendasDelta}
            sub={`ticket médio ${fmtVBR(FIN_RESUMO.bruto / FIN_RESUMO.vendas, true)}`}
            delay={120}
          />
        </div>
        ...
        <div className="stat-chip-grid" style={{ marginBottom: 32 }}>
          <StatChip
            ...
            value={fmtVBR(FIN_RESUMO.taxaPlataforma)}
            tone="neg"
          />
          <StatChip
            label="Ticket médio bruto"
            value={fmtVBR(FIN_RESUMO.ticketMedio)}
            tone="info"
          />
          <StatChip
            label="Em negociação"
            value={fmtVBR(FIN_RESUMO.emNegociacaoValor)}
            tone="warn"
          />
          <StatChip
            label="Qtd. em negociação"
            value={fmtNum(FIN_RESUMO.emNegociacaoQtd)}
            tone="warn"
          />
          <StatChip
            label="Comissão SDR"
            value={fmtVBR(FIN_RESUMO.comissaoSDR)}
            tone="neg"
          />
          <StatChip
            label="Comissão Closer"
            value={fmtVBR(FIN_RESUMO.comissaoCloser)}
            tone="neg"
          />
          <StatChip
            label="Total comissões"
            value={fmtVBR(FIN_RESUMO.comissaoTotal)}
            tone="neg"
          />
          <StatChip
            label="Margem op. (R$)"
            value={fmtVBR(FIN_RESUMO.margemOpValor)}
            tone="pos"
          />
        </div>
        ...
        <div className="section-head">
          <div>
            <div className="eyebrow">Receita mensal</div>
            <h2 className="section-title">Evolução, previsão vs realizado</h2>
          </div>
        ...
        {/* ── Receita por produto + por canal (split 2 cols) ── */}
        <div className="grid" style={{ marginBottom: 32 }}>
          <div className="card span-6" style={{ padding: '26px 28px' }}>
            <div className="card-header" style={{ marginBottom: 18 }}>
              ...
              <div className="hint">{PRODUTOS.length} produtos · {fmtVBR(PRODUTOS_TOTAL.bruto, true)} bruto</div>
            </div>
            <DonutProdutos data={PRODUTOS} />
          </div>
          ...
          <div className="card span-6" style={{ padding: '26px 28px' }}>
            ...
            <CanaisBarras data={RECEITA_POR_CANAL} />
          </div>
        </div>
        ...
        <div className="section-head">
          <div>
            ...
            <h2 className="section-title">Do bruto ao líquido</h2>
          </div>
...
const rootV = ReactDOM.createRoot(document.getElementById('root'));
rootV.render(<AppVendas />);
