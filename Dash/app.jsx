// Dashcomp — Visão geral (central de comando comercial)
  ...
  return (
    <div className="opcard" style={{ animationDelay: `${delay}ms` }}>
      <div className="opcard-head">
        <div className="op-ico" style={{ color: accent }}>
      ...
      <div className="op-bar">
        <div
          ...
          style={{
            width: `${pctClamped}%`,
            background: atingido ? 'var(--success)' : accent,
          }}
        />
      </div>
...
// ── Period sub-toggle (inside cards section) ─────
...
function App() {
  const [topPeriod, setTopPeriod] = useS('mes');           // high-level filter
  const [opsPeriod, setOpsPeriod] = useS('mes');           // operational cards period
  const [vendorFilter, setVendorFilter] = useS('todos');
  const [hallMode, setHallMode] = useS('closer');

  const vendorById = useM(() => Object.fromEntries(VENDEDORES.map(v => [v.id, v])), []);
  const sdrById = useM(() => Object.fromEntries(SDRS.map(v => [v.id, v])), []);

  const ranking = useM(() => {
    return RANKING_COTA.slice().sort((a, b) => b.pct - a.pct);
  }, []);
  const rankingTop3 = ranking.slice(0, 3);
  const sdrTop3 = SDR_RANKING.slice().sort((a, b) => b.score - a.score).slice(0, 3);

  const ops = OPS_METRICS[opsPeriod];
  const metaPct = (META_ATINGIDA / META_TOTAL) * 100;

  const hallData = hallMode === 'closer' ? rankingTop3 : sdrTop3;
  const hallPeople = hallMode === 'closer' ? vendorById : sdrById;
  const hallMetric = hallMode === 'closer'
    ? (r) => `${r.pct.toFixed(1)}%`
    : (r) => `${r.score.toFixed(1)} pts`;
  const hallSubMetric = hallMode === 'closer'
    ? (r) => kmoney(r.fat)
    : (r) => `${r.reunioes} reuniões`;
    ...
    <div className="dash">
      ...
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <nav className="topnav">
            <a className="item active" href="Dashcomp.html">Visão geral</a>
      ...
      <main className="main">
        ...
        <div className="grid stg" style={{ marginBottom: 28 }}>
          ...
          <div className="card span-5 hero">
            ...
            <div className="hero-foot">
              <div>
                <div className="lbl">Atingido</div>
                <div className="val num">{metaPct.toFixed(1)}%</div>
              </div>
              <div>
                <div className="lbl">Falta</div>
                <div className="val num">{kmoney(META_TOTAL - META_ATINGIDA)}</div>
              </div>
              <div>
                <div className="lbl">Meta</div>
                <div className="val num">{kmoney(META_TOTAL)}</div>
              </div>
              <div>
                <div className="lbl">Ritmo diário necessário</div>
                <div className="val num">{kmoney((META_TOTAL - META_ATINGIDA) / 7)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="card span-3 sales-card">
            ...
            <div className="sales-big num">{ops.vendas.realizado}</div>
            <div className="sales-sub">
              de <span className="num">{ops.vendas.meta}</span> vendas previstas ·&nbsp;
              <span className="num" style={{ color: 'var(--n-900)', fontWeight: 500 }}>
                {((ops.vendas.realizado / ops.vendas.meta) * 100).toFixed(0)}%
              </span>
            </div>
            <div className="op-bar" style={{ marginTop: 18 }}>
              <div className="op-fill" style={{
                width: `${Math.min((ops.vendas.realizado / ops.vendas.meta) * 100, 100)}%`,
                background: 'var(--b-500)',
              }} />
            </div>
            <div className="sales-dots">
              {Array.from({ length: ops.vendas.meta }).map((_, i) => (
                <span key={i} className={i < ops.vendas.realizado ? 'on' : ''} />
              ))}
            </div>
          </div>
          ...
          <div className="card span-4 hall-card">
            ...
            <div className="hall-footnote">
              {hallMode === 'closer'
                ? 'Ranking por % da cota atingida'
                : 'Ranking por performance de pré-vendas'}
            </div>
          </div>
        </div>

        {/* ── Row 2 — Operacional (pré-vendas) ────── */}
        ...
        <div className="ops-grid stg" style={{ marginBottom: 28 }}>
          <OpCard icon="hash"    label="Leads captados"      value={ops.numeros.realizado}    meta={ops.numeros.meta}    accent="#10B981" delay={40}  />
          <OpCard icon="phone"   label="Ligações agendadas"  value={ops.ligacoes.realizado}   meta={ops.ligacoes.meta}   accent="#2551D8" delay={100} />
          <OpCard icon="cal"     label="Reuniões agendadas"  value={ops.reunioes.realizado}   meta={ops.reunioes.meta}   accent="#3B6EF0" delay={160} />
          <OpCard icon="share"   label="Indicações captadas" value={ops.indicacoes.realizado} meta={ops.indicacoes.meta} accent="#8B5CF6" delay={220} />
        </div>

        {/* ── Row 3 — Ranking Closers (tabela completa) ────── */}
        <div className="grid stg" style={{ marginBottom: 28 }}>
            ...
            <table className="tbl closers-tbl" style={{ marginTop: 14 }}>
              ...
              <tbody>
                {ranking.map((r, i) => {
                  ...
                  const ops = CLOSER_OPS[r.id] || {};
                  const pctClamped = Math.min(r.pct, 150);
                  const lrPct = ops.la > 0 ? (ops.lr / ops.la) * 100 : 0;
                  const rrPct = ops.ra > 0 ? (ops.rr / ops.ra) * 100 : 0;
                  return (
                    <tr key={r.id}>
                      ...
                      <td className="right c-op">
                        <span className="la-pill num">{ops.la}</span>
                      </td>
                      ...
                      <td className="right c-op num">{ops.ra}</td>
                      <td className="right c-op num">
                        <span className="op-stack">
                          <span>{ops.rr}</span>
                      ...
                      <td className="right c-op num" style={{ color: 'var(--n-700)' }}>{ops.ind}</td>
                      ...
                      <td className="right" style={{ paddingRight: 24 }}>
                        <div className="heat">
                          ...
                          <span style={{
                            color: r.pct >= 100 ? 'var(--success)' : 'var(--n-700)',
                            fontWeight: r.pct >= 100 ? 600 : 500,
                            minWidth: 52,
                          }}>
                            {r.pct.toFixed(1)}%
                          </span>
        ...
        <div className="grid stg">
          <div className="card span-12 flush">
            ...
            <table className="tbl closers-tbl" style={{ marginTop: 14 }}>
              ...
              <tbody>
                {SDR_RANKING.slice().sort((a, b) => b.ligacoes - a.ligacoes).map((r, i) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td className="right c-op">
                        <span className="la-pill num">{r.ligacoes}</span>
                      </td>
                      <td className="right" style={{ paddingRight: 24 }}>
