...
export function useVendasData() {
  ...
  useEffect(() => {
    ...
    if (!apiKey) {
      console.warn('[useVendasData] VITE_SALES_API_KEY not set — skipping request')
      setData(null)
      setError('API key ausente. Configure VITE_SALES_API_KEY.')
      setLoading(false)
      return
    }
    ...
    apiGet('/sales/values', buildParams(filters), { headers: { 'X-API-Key': apiKey } })
      .then(d => {
