  ...
  return (
    <div className="pv-kpi" style={{ animationDelay: `${delay}ms` }}>
      ...
      <div className="op-bar" style={{ marginTop: 10 }}>
        <div className="op-fill" style={{ width: `${Math.min(pct, 100)}%`, background: atingido ? 'var(--success)' : cor }} />
      </div>
          ...
          return (
            <g key={s.id}>
              <polygon
                ...
                opacity={baseOpacity}
              />
              <text x={cx} y={y + rowH/2 - 4} textAnchor="middle" className="fnl-stg-name" fill="#fff">{s.nome}</text>
              <text x={cx} y={y + rowH/2 + 13} textAnchor="middle" className="fnl-stg-val" fill="#fff">{fmtPV(s.v)}</text>
              {convNext !== null && (
                <g>
                  <line x1={cx + wBot/2 + 6} y1={yBot} x2={W - 6} y2={yBot} stroke="var(--n-200)" strokeDasharray="2 3" />
                  <text x={W - 6} y={yBot - 6} textAnchor="end" className={`fnl-conv ${isWorst ? 'warn' : ''}`}>{convNext.toFixed(1)}%</text>
                </g>
              )}
              <text x={6} y={y + rowH/2 + 4} textAnchor="start" className="fnl-acum">{convTopo.toFixed(1)}%</text>
            </g>
          )
        })}
      </svg>
  ...
  return (
    <aside className="fnl-aside">
      ...
      <div className="fnl-summary">
        <div className="fnl-sum-cell"><div className="lbl">Topo do funil</div><div className="val num">{fmtPV(topo)}</div></div>
        <div className="fnl-sum-cell"><div className="lbl">Fechamentos</div><div className="val num" style={{ color: canal.cor }}>{fmtPV(fim)}</div></div>
        <div className="fnl-sum-cell"><div className="lbl">Conv. acumulada</div><div className="val num">{convAcum.toFixed(2)}%</div></div>
      </div>
      <div className="fnl-highlight warn">
        <div className="hl-icon">▾</div>
        <div>
          <div className="hl-label">Maior gargalo</div>
          <div className="hl-body"><strong>{worst.from.nome}</strong> → <strong>{worst.to.nome}</strong></div>
          <div className="hl-sub">Avanço de apenas <span className="num">{worst.pct.toFixed(1)}%</span> · perda de <span className="num">{fmtPV(worst.from.v - worst.to.v)}</span></div>
        </div>
      </div>
      <div className="fnl-highlight good">
        ...
        <div>
          ...
          <div className="hl-body"><strong>{best.from.nome}</strong> → <strong>{best.to.nome}</strong></div>
          <div className="hl-sub">Avanço de <span className="num">{best.pct.toFixed(1)}%</span></div>
        </div>
      </div>
          ...
          {s.map((e, i) => {
            ...
            return (
              <tr key={e.id}>
                ...
                <td className="right c-op num" style={{ color: 'var(--n-500)' }}>{i === 0 ? '—' : fmtPV(queda)}</td>
                <td className="right" style={{ paddingRight: 24 }}>
                  <div className="etp-bar">
                    <div className="etp-fill" style={{ width: `${convTopo}%`, background: canal.cor, opacity: 0.85 - i * 0.05 }} />
                  </div>
                </td>
              </tr>
            )
      ...
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <nav className="topnav">
            ...
            <a className="item active" href="/prevendas.html">Pré-vendas</a>
